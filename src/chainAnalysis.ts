...
export function printChainAnalysis(analysis: ChainAnalysis): void {
  console.log('\n🔗 DEEP CHAIN ANALYSIS');
  console.log(
    '================================================================================'
  );

  // Print goal strategies
  console.log('\n🎯 GOAL STRATEGIES:');
  for (const [goal, strategy] of analysis.goalStrategies) {
    console.log(`\n📋 ${goal}:`);
    console.log(`   Best strategy: ${strategy.bestStrategy.process}`);
    console.log(`   Total value: ${strategy.totalValue}`);
    console.log(`   Time to complete: ${strategy.totalTime} cycles`);
    console.log(`   Complexity: ${strategy.totalComplexity}`);
  ...
  // Print scale requirements
  console.log('\n📊 SCALE REQUIREMENTS:');
  for (const [resource, requirement] of analysis.scaleRequirements) {
    console.log(`\n🔸 ${resource}:`);
    console.log(`   Target: ${requirement.targetQuantity}`);
    console.log(`   Current: ${requirement.currentQuantity}`);
    console.log(`   Deficit: ${requirement.deficit}`);
    console.log(`   Time to produce: ${requirement.timeToProduce} cycles`);

    if (requirement.requiredProcesses.size > 0) {
      console.log(`   Required processes:`);
      for (const [process, count] of requirement.requiredProcesses) {
        console.log(`     • ${process}: ${count} times`);
      }
    }
  }

  // Print top resource paths
  console.log('\n🛤️  TOP RESOURCE PATHS:');
  const sortedPaths = Array.from(analysis.resourcePaths.values())
    .sort((a, b) => b.totalValue - a.totalValue)
    .slice(0, 10);

  for (const path of sortedPaths) {
    console.log(`\n🔸 ${path.resource}:`);
    console.log(`   Total value: ${path.totalValue}`);
    console.log(`   Direct paths: ${path.directPaths}`);
    console.log(`   Chain paths: ${path.chainPaths}`);
    console.log(`   Required scale: ${path.requiredScale}`);
    console.log(`   Max chain depth: ${path.maxChainDepth}`);
  }

  // Print top process strategies
  console.log('\n⚙️  TOP PROCESS STRATEGIES:');
  const sortedStrategies = Array.from(analysis.processStrategies.values())
    .sort(
      (a, b) => b.directValue + b.chainValue - (a.directValue + a.chainValue)
    )
    .slice(0, 10);
  ...
  for (const strategy of sortedStrategies) {
    console.log(`\n🔸 ${strategy.process}:`);
    console.log(`   Direct value: ${strategy.directValue}`);
    console.log(`   Chain value: ${strategy.chainValue}`);
    console.log(
      `   Total value: ${strategy.directValue + strategy.chainValue}`
    );
    console.log(`   Production steps: ${strategy.productionSteps}`);
    console.log(`   Time to complete: ${strategy.timeToComplete} cycles`);
    console.log(`   Complexity: ${strategy.complexity}`);

    if (strategy.scaleMultiplier > 1) {
      console.log(`   Scale multiplier: ${strategy.scaleMultiplier}x`);
    }
  }
}
